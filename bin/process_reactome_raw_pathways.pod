...
=item --no-add-pseudonodes

By default we will be adding pseudonodes. If you would like to skip this step add this flag.

=item --max-number-of-parents [=] <max_number_of_parents>

This will be used for adding parents when the number of parents to a node exceeds the max. 
