...
hashform *form;
{
  ...
  fprintf(f, "#define MPH_SALT 0x%.8" PRIx32 " /* internal, initialize normal hash */\n",
           salt*0x9e3779b9);
  fprintf(f, "\n");
  fprintf(f, "uint32_t phash_s();\n");
  fprintf(f, "\n");
  fprintf(f, "#define phash(key) phash_s((key), (uint32_t)strlen((key)))\n");
  fprintf(f, "\n");
  fprintf(f, "#endif  /* MPH */\n");
  fprintf(f, "\n");
  fclose(f);
...
"0x%.4" PRIx32 ", 0x%.4" PRIx32 ", 0x%.4" PRIx32 ", 0x%.4" PRIx32 ", 0x%.4" PRIx32 ", 0x%.4" PRIx32 ", 0x%.4" PRIx32 ", 0x%.4" PRIx32 ",\n",
  ...
  {
  case NORMAL_HM:
    fprintf(f, "uint32_t phash_s(char *key, uint32_t len)\n");
    break;
  case INLINE_HM:
  case HEX_HM:
  case DECIMAL_HM:
    fprintf(f, "uint32_t phash_s(uint32_t val)\n");
    break;
  case AB_HM:
  case ABDEC_HM:
    fprintf(f, "uint32_t phash_s(uint32_t a, uint32_t b)\n");
    break;
  }
  fprintf(f, "{\n");
